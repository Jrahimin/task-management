...
<script>
    ...
    export default {
        ...
        created() {
            axios.get('recurring/'+this.workOrderId).then(response => {
        ...
        methods:{
            changeStatus(){
                axios.post('recurring-job/change-status/'+this.workOrderId, {status: this.status,users:this.user_ids}).then(response =>{
                    this.$router.go();
                });
            },
        }
